...
//------------------------Parameter----------------------
`define TV_IN_vec_a "../tv/cdatafile/c.multiplier.autotvin_vec_a.dat"
`define TV_IN_vec_b "../tv/cdatafile/c.multiplier.autotvin_vec_b.dat"
`define TV_IN_result "../tv/cdatafile/c.multiplier.autotvin_result.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter vec_a_DEPTH = 1;
reg [31 : 0] vec_a_OPERATE_DEPTH = 0;
parameter vec_a_c_bitwidth = 64;
parameter vec_b_DEPTH = 1;
reg [31 : 0] vec_b_OPERATE_DEPTH = 0;
parameter vec_b_c_bitwidth = 64;
parameter result_DEPTH = 1;
reg [31 : 0] result_OPERATE_DEPTH = 0;
parameter result_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter multiplier_continue_addr = 0;
parameter multiplier_auto_start_addr = 0;
parameter vec_a_data_in_addr = 16;
parameter vec_b_data_in_addr = 28;
parameter result_data_in_addr = 40;
parameter vector_len_data_in_addr = 52;
...
reg  BREADY_reg = 0;
reg [vec_a_c_bitwidth - 1 : 0] mem_vec_a [vec_a_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_vec_a [ (vec_a_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * vec_a_DEPTH -1 : 0] = '{default : 'hz};
reg vec_a_write_data_finish;
reg [vec_b_c_bitwidth - 1 : 0] mem_vec_b [vec_b_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_vec_b [ (vec_b_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * vec_b_DEPTH -1 : 0] = '{default : 'hz};
reg vec_b_write_data_finish;
...
//write vec_a reg
reg [31 : 0] write_vec_a_count = 0;
reg write_vec_a_run_flag = 0;
reg write_one_vec_a_data_done = 0;
//write vec_b reg
reg [31 : 0] write_vec_b_count = 0;
reg write_vec_b_run_flag = 0;
reg write_one_vec_b_data_done = 0;
//write result reg
...
assign TRAN_control_write_data_finish = 1 & vec_a_write_data_finish & vec_b_write_data_finish & result_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        vec_a_write_data_finish <= 0;
        write_vec_a_run_flag <= 0; 
        write_vec_a_count = 0;
        count_operate_depth_by_bitwidth_and_depth (vec_a_c_bitwidth, vec_a_DEPTH, vec_a_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_vec_a_run_flag <= 1; 
            write_vec_a_count = 0;
        end
        if (write_one_vec_a_data_done === 1) begin
            write_vec_a_count = write_vec_a_count + 1;
            if (write_vec_a_count == vec_a_OPERATE_DEPTH) begin
                write_vec_a_run_flag <= 0; 
                vec_a_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_vec_a
    integer write_vec_a_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] vec_a_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = vec_a_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_vec_a_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write vec_a data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (vec_a_c_bitwidth < 32) begin
                        vec_a_data_tmp_reg = mem_vec_a[write_vec_a_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < vec_a_c_bitwidth) begin
                                vec_a_data_tmp_reg[j] = mem_vec_a[write_vec_a_count][i*32 + j];
                            end
                            else begin
                                vec_a_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_vec_a[write_vec_a_count * four_byte_num  + i]!==vec_a_data_tmp_reg) begin
                    write (vec_a_data_in_addr + write_vec_a_count * four_byte_num * 4 + i * 4, vec_a_data_tmp_reg, write_vec_a_resp);
                    image_mem_vec_a[write_vec_a_count * four_byte_num + i]=vec_a_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_vec_a_data_done <= 1;
                @(posedge clk);
                write_one_vec_a_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        vec_b_write_data_finish <= 0;
        write_vec_b_run_flag <= 0; 
        write_vec_b_count = 0;
        count_operate_depth_by_bitwidth_and_depth (vec_b_c_bitwidth, vec_b_DEPTH, vec_b_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            vec_b_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_vec_b_run_flag <= 1; 
            write_vec_b_count = 0;
        end
        if (write_one_vec_b_data_done === 1) begin
            write_vec_b_count = write_vec_b_count + 1;
            if (write_vec_b_count == vec_b_OPERATE_DEPTH) begin
                write_vec_b_run_flag <= 0; 
                vec_b_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_vec_b
    integer write_vec_b_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] vec_b_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = vec_b_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_vec_b_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write vec_b data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (vec_b_c_bitwidth < 32) begin
                        vec_b_data_tmp_reg = mem_vec_b[write_vec_b_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < vec_b_c_bitwidth) begin
                                vec_b_data_tmp_reg[j] = mem_vec_b[write_vec_b_count][i*32 + j];
                            end
                            else begin
                                vec_b_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_vec_b[write_vec_b_count * four_byte_num  + i]!==vec_b_data_tmp_reg) begin
                    write (vec_b_data_in_addr + write_vec_b_count * four_byte_num * 4 + i * 4, vec_b_data_tmp_reg, write_vec_b_resp);
                    image_mem_vec_b[write_vec_b_count * four_byte_num + i]=vec_b_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_vec_b_data_done <= 1;
                @(posedge clk);
                write_one_vec_b_data_done <= 0;
            end   
            process_2_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_vec_a_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [vec_a_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (vec_a_c_bitwidth , factor);
  fp = $fopen(`TV_IN_vec_a ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_vec_a); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < vec_a_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_vec_a [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
...
// Read data from file 
initial begin : read_vec_b_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [vec_b_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (vec_b_c_bitwidth , factor);
  fp = $fopen(`TV_IN_vec_b ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_vec_b); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < vec_b_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_vec_b [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
